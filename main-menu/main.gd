extends Control


func _on_demo_01_button_pressed() -> void:
	Global.change_scene("res://game-01/main.tscn")


func _on_demo_02_button_pressed() -> void:
	Global.change_scene("res://game-02/main.tscn")


func _on_demo_03_button_pressed() -> void:
	Global.change_scene("res://game-03/main.tscn")


func _on_demo_04_button_pressed() -> void:
	Global.change_scene("res://game-04/main.tscn")


func _on_demo_05_button_pressed() -> void:
	Global.change_scene("res://game-05/main.tscn")
